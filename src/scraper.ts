  ...
  const rawData = await page.evaluate(
    () => {
      ...
      for (let i = 0; i < threads.length; i++) {
        ...
        // Query elements directly from the thread wrapper.
        // querySelector naturally returns the first instance, which corresponds to the main comment.
        const textEl = thread.querySelector(
          '#content-text'
        );

        // If there's no content text, it's not a valid comment
        if (!textEl) {
          continue;
        }
        ...
        const authorEl = thread.querySelector(
          '#author-text'
        );
        ...
        const likesEl = thread.querySelector(
          '#vote-count-middle'
        );
        ...
        if (repliesContainer) {
          // We only want the #content-text elements found INSIDE the replies container
          const replyEls = repliesContainer.querySelectorAll(
            '#content-text'
          );

          for (let j = 0; j < replyEls.length; j++) {
            const replyText = (replyEls[j] as HTMLElement).innerText;
